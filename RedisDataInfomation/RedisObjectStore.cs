...
namespace RedisCacheDemo
{
    ...
    public class RedisObjectStore<T> : IRedisObjectStore<T>
    {
        ...
        public T Get(string key)
        {
            ...
            return MapFromHash2(hash);
        }
        public T Get2(string key)
        {
        ...
        HashEntry[] GenerateRedisHash(T obj)
        {
            ...
            var json = new NewtonJsonTester<T>(obj);
        ...
        //build object from hash entry array using reflection
        private static T MapFromHash2(HashEntry[] hash)
        {
            ...
            for (int i = 0; i < props.Count(); i++)
                for (int j = 0; j < hash.Count(); j++)
                    if (props[i].Name == hash[j].Name)
                    {
                        var val = (string)hash[j].Value;
                        var type = props[i].PropertyType;
                        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable))
                        {
                            var safeValue = string.IsNullOrEmpty(val) ? null : Convert.ChangeType(val, Nullable.GetUnderlyingType(type));
                            props[i].SetValue(obj, safeValue, null);
                        }
                        else
                        {
                            if(type.Name == "Guid")
                                props[i].SetValue(obj, Guid.Parse(val));
                            else
                            props[i].SetValue(obj, Convert.ChangeType(val, type));
                        }
                    }
